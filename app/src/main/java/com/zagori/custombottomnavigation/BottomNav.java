...
public class BottomNav extends CoordinatorLayout {
    ...
    public BottomNav(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        inflate(getContext(), R.layout.custom_bottom_navigation, this);
        ...
        Float fabSize = styleAttrs.getDimension(R.styleable.CustomBottomNavigation_bn_fab_size, 0F);
        Float fabMargin = styleAttrs.getDimension(R.styleable.CustomBottomNavigation_bn_fab_margin, 0F);
        Float bottomNavRoundedCornerRadius = styleAttrs.getDimension(R.styleable.CustomBottomNavigation_bn_rounded_corner_radius, 0F);
        Float cradleVerticalOffset = styleAttrs.getDimension(R.styleable.CustomBottomNavigation_bn_cradle_vertical_offset, 0F);
        @ColorInt int bottomNavBackgroundColor = styleAttrs.getColor(R.styleable.CustomBottomNavigation_bn_background_color, bottomNavigationView.getSolidColor()); // bottomNavigationView.getItemBackground()
        @IdRes int menuResID = styleAttrs.getResourceId(R.styleable.CustomBottomNavigation_bn_menu, -1);
        final int fabMenuIndex = styleAttrs.getInt(R.styleable.CustomBottomNavigation_bn_fab_menu_index, -1);
        ...
        if (fabMenuIndex >= bottomNavigationView.getMenu().size()){
            fab.setVisibility(GONE);
            Log.e(TAG, "fabMenuIndex is out of bound. fabMenuIndex only accepts values from 0 to " +
                    Integer.toString(bottomNavigationView.getMenu().size()-1));
        ...
        TopEdgeTreatment topEdgeTreatment = new TopEdgeTreatment(bottomNavigationView.getMenu().size(), fabMenuIndex, fabMargin, bottomNavRoundedCornerRadius, cradleVerticalOffset);
        ...
        materialShapeDrawable.setTint(bottomNavBackgroundColor);
